...
async def _ai_parse_flyer_image(flyer_image: UploadFile, item_type: str) -> AIFlyerExtractionResponse:
...
async def ai_parse_flyer(
    ...
    """
    data = await _ai_parse_flyer_image(flyer_image, "event")
...
async def ai_parse_activity_flyer(
    flyer_image: UploadFile = File(...),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    data = await _ai_parse_flyer_image(flyer_image, "activity")
